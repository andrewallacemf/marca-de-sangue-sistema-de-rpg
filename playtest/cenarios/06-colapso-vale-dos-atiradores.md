# Cenário 6 — O Vale dos Atiradores

**Ambientação:** Colapso (pós-apocalíptico junkpunk) · **Fichas:** 50 exp · **Jogadores:** 4 · **Missão:** incursão sob fogo

> ♻️ **Fichas rebalanceadas (playtest 1 → custo escalonado).** Carregue os `.mds.json`; os
> blocos em prosa abaixo são referência e podem diferir.
> ⚙️ **Regras novas:** sair do engajamento provoca AoO (desengajar 1 PA + 1/inimigo); todo
> dano gera 1 fadiga/ponto; membro enche em 10 → profundo. **HP de minion:** fraco 10 · médio
> 20 · forte 30. Olhos de Chumbo = **médio (20)**; Cães da Gorja = **médio (20)**; Vigia-mor
> = **forte (30)**.

> 🧪 **Cenário-teste da COBERTURA** (proposta de Daniel). Ele existe para estressar a
> mecânica de [cobertura](../../sistema-base/conflitos/01-conflito-fisico.md#cobertura),
> a [elevação](../../sistema-base/conflitos/01-conflito-fisico.md#elevação) e o
> [terreno difícil](../../sistema-base/conflitos/01-conflito-fisico.md#condições-de-ambiente-e-terreno),
> além de forçar a decisão **se cobrir × avançar × lutar**.

## Situação (para o narrador)

Para chegar ao velho **relé de água** no topo da **Gorja** — a única fonte limpa da região —,
o grupo tem que subir uma ravina estreita e exposta. Os dois espinhaços que ladeiam a Gorja
estão ocupados pelos **Olhos de Chumbo**, atiradores que dominam o vale de cima. No chão da
ravina há **cobertura de sobra** (carcaças de carro, muros de concreto, tambores, contêineres),
mas o caminho é longo e cortado por **destroços** e um trecho **alagado**. No meio da subida,
uma matilha de saqueadores — os **Cães da Gorja** — espreita numa manilha lateral, pronta para
emboscar quem parar tempo demais.

Não é para ser só "corre e esconde": a cada avanço o grupo escolhe entre **ficar colado na
cobertura** (seguro dos atiradores, mas sem progredir) e **se expor para andar/atirar/lutar**.
Quando os Cães atacam no corpo a corpo, o dilema aperta — quem sai da cobertura para revidar
vira alvo fácil lá de cima.

## A malha

Ravina de **18 espaços de comprimento** por 8 de largura. Borda esquerda = **entrada** (embaixo),
borda direita = **relé** (topo). As **linhas de cima e de baixo** da malha são os espinhaços,
**+2 m de elevação**, onde ficam os atiradores.

```
 espinhaço (atiradores ▲, elevação +2m, com cobertura própria) ─────────────────
[ENTRADA]  ▮muro▮   █auto█  ░░destroços░░   ~~alagado~~   ▮contêiner▮   █auto█  [RELÉ]
 J1..J4    (compl.) (meia)   (t. difícil)    (t. difícil)   (completa)   (meia)   ← objetivo
                         ╔═manilha═╗  ← os Cães da Gorja emboscam aqui
 espinhaço (atiradores ▲, elevação +2m, com cobertura própria) ─────────────────
```

Legenda da cobertura:

- **▮ / contêiner / muro = cobertura completa** — com a **guarda levantada** ali, os atiradores
  **não acertam**. Sem guarda (exposto, andando ou atirando), vale como **meia cobertura**.
- **█ auto / tambor = meia cobertura** — **`+1d10`** na defesa contra tiros, mas ainda pode ser atingido.
- **░░ destroços ░░** e **~~alagado~~** = **terreno difícil**: deslocamento custa **o dobro** de PA.
- **▲ elevação (+2m):** os atiradores no alto atiram **com vantagem**; quem tenta acertá-los
  **de baixo atira com desvantagem**. Subir a rampa até um espinhaço custa o PA da elevação.

## Como a cobertura entra em jogo

- **Avançar entre coberturas:** ande até uma cobertura e, **ao fim do turno, levante a guarda**
  (2 PA fora de engajamento) para se abrigar. No turno seguinte, ou fica seguro, ou gasta a
  guarda saindo para andar/atirar — e se expõe.
- **Contra-atirar:** atirar de dentro de uma cobertura completa **expõe** o personagem (vale meia
  cobertura) até a próxima guarda. Franco-atiradores do grupo precisam escolher a janela.
- **Duplo dilema no meio:** quando os Cães atacam corpo a corpo, a cobertura **não vale contra
  eles** (corpo a corpo ignora cobertura). Quem vira para lutar larga a guarda e fica exposto aos
  atiradores do alto — é aí que **Torre** (barricada) e as armadilhas de **Eco** ganham o jogo.

## Inimigos

- **3 Olhos de Chumbo** (atiradores nos espinhaços) — PA 10 · Rifle (pesada · 6 PA · alcance 3–20 m ·
  bala `1d8`) · usam **cobertura completa + elevação**: atiram com vantagem e se abaixam entre tiros.
  Para silenciá-los é preciso subir a rampa (flanquear) ou acertá-los na janela em que se expõem.
- **4 Cães da Gorja** (emboscada na manilha, no meio) — PA 10 · cano de ferro (arma improvisada
  média · contundente 3 · 4 PA) · surgem quando o grupo chega ao trecho de terreno difícil;
  tentam agarrar e prender quem estiver isolado.
- *(Opcional)* **1 Vigia-mor** — PA 12 · besta pesada · coordena os atiradores; some se o grupo
  tomar um espinhaço.

## Objetivo

- **Vitória:** os **quatro** personagens alcançam o relé (borda direita).
- **Vitória plena (bônus de teste):** tomar pelo menos **um espinhaço** (incapacitar/afugentar os
  atiradores de um lado) antes de cruzar — reduz o fogo sobre a reta final.
- **Derrota:** dois personagens incapacitados e presos pelos Cães sem resgate.

## Dica de teste (o que observar)

- A **cobertura** cria decisões interessantes ou vira "ficar parado atrás do muro"? O custo da
  guarda (2/3 PA) para se cobrir está bom, ou deveria ser o "1 PA para se abaixar" do áudio?
- **Meia vs completa** ficou claro na mesa? O "expõe ao atirar" funciona sem burocracia?
- **Elevação + cobertura** juntas: o fogo de cima é ameaçador o suficiente para valer a cobertura,
  sem ser injusto?
- O **terreno difícil** (dobro de custo) e a **emboscada** no meio realmente forçam a escolha
  entre se cobrir e avançar/lutar?

---

## Ficha 1 — FIO, a Franco-atiradora

*Responde aos espinhaços. Da cobertura, escolhe a janela e derruba um atirador por vez.*

- **Mão hábil:** direita · **PA por turno:** **10** · **PV:** 60 (10 por membro)
- **Armas:** Rifle (pesada · à distância · 6 PA · alcance 3–20 m · bala `1d8` · `PROJ`) · Pistola simples (leve · reserva · 2 PA · 2–15 m · `1d8`)
- **Proteção:** nenhuma (mobilidade e furtividade)

**Características (50 exp):**

- Maestria em projéteis (1) — ignora barreiras leves; recarregar não custa PA ao acertar
- Técnica em Esconder-se (1)
- Lépido (3) — recua 1 m após acertar com arma de longo alcance (reposiciona a cobertura)
- Escorregadio (4) — +1d10 para esquivar de ataques de oportunidade
- Ataques à distância concentrados — nível 3 (6) — mantém o mesmo atirador na mira sem penalidade
- Alcance à distância — nível 2 (4) — amplia a faixa ideal do rifle
- Analisar fraquezas — nível 3 (3) — mira onde o alvo já apanhou
- Disparada — nível 1 (2) — reposiciona rápido entre coberturas
- Aptidão Ágil ×20 (20) · Aptidão Mental ×6 (6)

**Como jogar:** você é o contrapeso dos espinhaços. Fique em **cobertura completa**, levante a
guarda e só **saia para atirar** quando valer — lembre que atirar te **expõe** até a próxima
guarda. **Analisar fraquezas + Ataques à distância concentrados** afunilam o dano num atirador
até derrubá-lo. **Lépido** te devolve para trás da cobertura depois do tiro.

---

## Ficha 2 — BRECHA, o Assaltante

*Fecha a distância de cobertura em cobertura e resolve a emboscada de perto.*

- **Mão hábil:** direita · **PA por turno:** **10** · **PV:** 60 (10 por membro)
- **Armas:** Espingarda (média · à distância · 4 PA · alcance 1–5 m · bala `1d8`) · Faca (leve · reserva · 1d4)
- **Proteção:** nenhuma (velocidade)

**Características (50 exp):**

- Maestria em projéteis (1)
- Escorregadio (4) — passa entre inimigos sem levar ataque de oportunidade
- Empacado (2) — os Cães não te arrastam para fora da linha
- Disparada — nível 3 (6) — salto de até 8 m em linha reta entre coberturas
- Investida — nível 3 (9) — avança ignorando engajamento e ataca (resolve a emboscada)
- Ataques à distância concentrados — nível 2 (4) — foca um alvo de perto
- Arremesso improvisado — nível 1 (2) — improvisa um arremesso quando precisar
- Aptidão Ágil ×16 (16) · Aptidão Física ×6 (6)

**Como jogar:** você é a ponta de lança. Use **Disparada** para cruzar os vãos expostos num
lance só (menos tempo sob mira) e chegar à próxima cobertura. Quando os **Cães** aparecerem,
**Investida** te leva até eles e a **espingarda de perto** limpa o corredor — poupando o resto do
grupo de largar a cobertura.

---

## Ficha 3 — TORRE, a Barricada

*Cobertura ambulante. Segura a emboscada e protege quem está atirando.*

- **Mão hábil:** direita · **PA por turno:** **6** (10 −2 peitoral −2 escudo) · **PV:** 60 (10 por membro)
- **Armas:** Martelo médio (média · contundente `2d4` · versátil · 4 PA) · Pistola simples (leve · reserva · 2 PA · 2–15 m · `1d8`)
- **Proteção:** Peitoral médio (−2 dano no tronco) · Escudo médio (−2 dano · levantar guarda 3 PA)

**Características (50 exp):**

- Maestria em armaduras médias (1) · Maestria em armas contundentes (1) — cada acerto tira 2 PA do alvo
- Blindado (4) — **+1d10 para defender ataques à distância com o escudo** (casa com a cobertura)
- Resistente (3) — reduz o dano recebido pela metade (com guarda + armadura média)
- Protetor (4) — intercepta para si um golpe que acertaria um aliado adjacente
- Reativo (4) — ataque de oportunidade quando um Cão sai do seu alcance
- Empacado (2) — não é empurrado/arrastado
- Atordoar — nível 2 (8) — tira PA persistente dos Cães
- Encontrão — nível 2 (4) — empurra para fora da linha
- Quebrar guarda — nível 1 (2)
- Aptidão Física ×17 (17)

**Como jogar:** você é a cobertura que anda. Poste-se à frente dos atiradores do grupo: com
**escudo + Blindado**, você e quem está atrás ganham defesa contra os tiros de cima. Quando os
**Cães** emboscam, **você** os enfrenta de perto (Atordoar/Encontrão) enquanto **Fio e Brecha**
seguem cuidando dos espinhaços. **Protetor** salva um aliado exposto de um tiro certeiro. PA baixo
(6): é o preço da lata — administre.

---

## Ficha 4 — ECO, o Coiote

*Move o grupo, arma o terreno e cura. Faz a incursão fluir.*

- **Mão hábil:** direita · **PA por turno:** **10** · **PV:** 60 (10 por membro)
- **Armas:** Pistola simples (leve · à distância · 2 PA · alcance 2–15 m · bala `1d8`) · Faca (leve · reserva · 1d4)
- **Proteção:** nenhuma

**Características (50 exp):**

- Técnica em Investigar/Observar (1) · Técnica em Realizar truques (1)
- Apoiador (4) — usa aptidões nos testes de um aliado adjacente
- Comandar — nível 3 (6) — transfere até 4 PA para quem precisa cruzar um vão
- Criar armadilha — nível 3 (6) — prepara a manilha contra os Cães (dano ao acionar)
- Cura especializada — nível 2 (6) — recoloca de pé quem apanhou dos atiradores
- Confundir inimigo — nível 3 (9) — impõe desvantagem no próximo tiro de um atirador
- Analisar fraquezas — nível 2 (2)
- Aptidão Social ×9 (9) · Aptidão Mental ×6 (6)

**Como jogar:** você é o maestro. **Comandar** dá PA extra para Brecha (ou qualquer um) cruzar um
trecho exposto num turno só. **Criar armadilha** na boca da manilha recebe os **Cães** antes que
cheguem. **Confundir inimigo** estraga o tiro do atirador que estiver mirando um aliado sem
cobertura. Se alguém cair na reta final, **Cura especializada** o levanta.
