---
titulo: Visão geral — Colapso
tipo: cenario
cenario: colapso
status: rascunho
tags: [colapso, visao-geral, junkpunk, pos-apocaliptico]
atualizado-em: 2026-07-19
---

# Colapso — Visão geral

## Pitch

**Junkpunk pós-apocalíptico** no Brasil de 2128+30: depois da invasão alienígena, sobraram
desertos radioativos, sucateiros, piratas de areia e o combate veicular em alta velocidade.
Personagens têm prazo de validade; veículos também.

## Tom e temas

Sobrevivência, escassez e improviso; estrada e velocidade; radiação e mutação; comunidades
frágeis e facções rivais. Referências: *Mad Max*, wasteland, junkpunk.

## O que este cenário adiciona ao sistema-base

- **Conflito de veículos** — perseguição e combate em alta velocidade, com dano ao veículo,
  manobras, colisão, invasão e equipamentos. A mecânica virou o
  [módulo `veiculos`](../../modulos/veiculos/README.md) em 01/08/2026; o cenário usa o módulo
  inteiro e acrescenta o que é seu (ver [veículos no Colapso](mecanicas-unicas/veiculos.md)).
- **Sucata** — a matéria-prima do ermo, que dá forma às *peças de reposição* do módulo de veículos
  e sustenta reparo e fabricação ([sucata](mecanicas-unicas/sucata.md)).
- **Radiação, ambiente e abrigo** — radiômetro, dano radioativo, hipotermia/insolação e níveis de
  abrigo ([radiação](mecanicas-unicas/radiacao.md)).
- **Mutações** (Cromática 1d100 e Púrpura 1d12) — ver [mutações](mecanicas-unicas/mutacoes.md).
  Candidato a módulo.
- **Armas de fogo pouco confiáveis** — durabilidade em 5 níveis e tiro pela culatra
  ([armas de fogo](mecanicas-unicas/armas-de-fogo.md)).
- **Listas próprias** — [habilidades](mecanicas-unicas/habilidades-do-cenario.md) e
  [traços](mecanicas-unicas/tracos-do-cenario.md) do cenário. O Colapso também **liga as duas
  mecânicas opcionais** do módulo de veículos: a **Aptidão Veicular** e a **Experiência de
  Veículos**.
- **Arquétipos do ermo** e de veículo — ver [arquétipos](arquetipos.md).

## O mundo (lore)

Em [`lore/`](lore/README.md): [o Colapso](lore/01-o-colapso.md),
[clima e radiação](lore/02-clima-e-radiacao.md), [comunidades](lore/03-comunidades.md) e
[gangues e facções](lore/04-faccoes.md). As regiões do ermo estão em
[locais](locais/README.md).

## O que muda em relação ao base

- Usa o módulo [armas-de-fogo](../../modulos/armas-de-fogo/README.md) (sobreposto pela regra de
  confiabilidade própria) e a [furtividade](../../modulos/furtividade/README.md); as armas vêm das
  famílias da coleção `armas` + improvisadas (ver [arsenal.md](arsenal.md)).
- Introduz uma segunda moeda de experiência, a **Experiência de Veículo**, ao lado da exp. padrão.

> ⚠️ A DEFINIR: bestiário, tabela de armas de fogo, Púrpuras 9–12 e reconciliação da exp. inicial
> de veículo (Gabriel). Detalhes em [ganchos](lore/ganchos-e-a-desenvolver.md).
