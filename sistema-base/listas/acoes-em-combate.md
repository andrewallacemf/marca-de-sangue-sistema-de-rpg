---
titulo: Lista de ações disponíveis em combate
tipo: regra
cenario: base
status: revisao
...
| Criar um gatilho | **`1 PA`** | por gatilho (+**`PA`** da ação, habilidade ou arma) |
| Executar uma ação complexa | **`Varia`** | por ação |
...
| **Ações** | **Atributo** | **Pontos** | **Unidade** |
| --- | --- | --- | --- |
| Agarrar | `Físico` | **`1 PA`** | por ação |
| Empurrar | `Físico` | **`1 PA`** | por ação |
| Arremessar | `Físico` | **`1 PA`** | a cada 3 metros |
...
>
> - **Técnica anula desvantagem, nunca concede vantagem** (decisão de 12/07/2026, tomada ao
>   redigir Saltar). Quando o efeito desejado era um bônus, ele virou nota para o grupo avaliar como
>   aspecto, não como técnica.
> - **O `PA` já está fixado** na tabela acima; as descrições respeitam esses custos sem alterá-los.

### Agarrar

**Atributo:** `Físico` · **Custo da técnica:** `1 exp.`
...
- Pelo menos **um braço** não [desabilitado](../conflitos/03-saude-e-protecao.md#membro-desabilitado-incapacitado-ou-invalidado) — ou uma arma **`AGAR.`** equipada.
...
**Atributo:** `Ágil` · **Custo da técnica:** *(não há técnica de Correr)*
...
- Nenhuma **perna** desabilitada.

### Derrubar

**Atributo:** `Físico` · **Custo da técnica:** *(não há técnica de Derrubar)*
...
**Atributo:** `Mental` · **Custo da técnica:** `1 exp.`
...
**Atributo:** `Físico` · **Custo da técnica:** *(não há técnica de Levantar)*
...
**Atributo:** `Social` ou `Mental` · **Custo da técnica:** `3 exp.`
...
- Não estar **inconsciente** nem com a **cabeça** desabilitada.

### Realizar Truques

**Atributo:** `Ágil` ou `Social` · **Custo da técnica:** `1 exp.`
