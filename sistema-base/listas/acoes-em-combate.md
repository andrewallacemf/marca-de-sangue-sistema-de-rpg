---
titulo: Lista de ações disponíveis em combate
tipo: regra
cenario: base
status: revisao
tags: [acoes, combate, pontos-de-acao]
atualizado-em: 2026-07-12
---

# Lista de ações disponíveis em combate

## Comprar pontos de ação

**No início da jornada, os personagens começam com `10 PA`.** Ao receberem pontos de experiência, podem adquirir pontos de ação adicionais. O primeiro ponto adicional custa **`5 exp.`** e cada ponto subsequente custa **`5 exp.`** a mais:

- + **`1 PA`** = **`5 exp.`**
- + **`1 PA`** = **`10 exp.`**
- + **`1 PA`** = **`15 exp.`**
- + **`1 PA`** = **`20 exp.`**
- + **`1 PA`** = **`25 exp.`**
- Assim em diante…

## Ações básicas

| **Ações** | **Pontos** | **Unidade** |
| --- | --- | --- |
| Movimento | **`1 PA`** | por metro (espaço da malha) — máx. `7 PA`/turno |
| Ataque desarmado | **`1 PA`** | por ataque |
| Ataque com arma leve | **`2 PA`** | por ataque |
| Ataque com arma média | **`4 PA`** | por ataque |
| Ataque com arma pesada | **`6 PA`** | por ataque |
| Entrar em postura defensiva (levantar guarda) sem estar engajado | **`2 PA`** | por guarda levantada |
| Entrar em postura defensiva (levantar guarda) quando engajado | **`3 PA`** | por guarda levantada |
| Forçar Posicionamento (aberto ou definido) | **`2 PA`** | por mudança de posicionamento |
| Atrasar turno | **`1 PA`** | por turno atrasado |
| Guardar ou sacar equipamentos | **`1 PA`** | por arma |
| Mudar a mão dos equipamentos | **`1 PA`** | por tentativa |
| Passar uma mensagem | **`1 PA`** | para cada 4 palavras |
| Agarrar | **`1 PA`** | por tentativa |
| Se soltar (quando agarrado/preso) | **`2 PA`** | por tentativa |
| Levantar (quando caído) | **`2 PA`** | por tentativa |
| Cura/tratamento improvisado | **`4 PA`** | por tentativa |
| Criar um gatilho | **`1 PA`** | por gatilho (+**`PA`** da ação, habilidade ou arma) |
| Executar uma ação complexa | **`Varia`** | por ação |
| Ativar/usar uma habilidade | **`Varia`** | varia de acordo com a habilidade |

\*Outros tipos de ação não previstos neste manual podem ter seu custo definido pelo narrador com base nas ações já definidas.

> ✅ Rebalanceamento de 12/07/2026: PA inicial 7 → **10**; custo de ataque por peso passa a **leve `2 PA` / média `4 PA` / pesada `6 PA`** (o desarmado segue `1 PA`); o custo é pelo **peso da arma**, valendo igual para corpo a corpo e à distância — em armas versáteis a empunhadura muda só o dano, não o `PA`. As demais ações mantêm o custo. As **habilidades de custo fixo** serão rebalanceadas numa passada seguinte (ver [decisões — rebalanceamento de PA](../../notas-de-design/decisoes/2026-07-12-rebalanceamento-pa-analise.md)).

## Ações complexas

| **Ações** | **Atributo** | **Pontos** | **Unidade** |
| --- | --- | --- | --- |
| Agarrar | `Físico` | **`1 PA`** | por ação |
| Empurrar | `Físico` | **`1 PA`** | por ação |
| Arremessar | `Físico` | **`1 PA`** | a cada 3 metros |
| Derrubar | `Físico` | **`3 PA`** | Por tentativa |
| Levantar | `Físico` | **`2 PA`** | Por ação |
| Saltar | `Ágil` | **`1 PA`** | por metro |
| Correr | `Ágil` | **`1 PA`** | a cada 2 metros em linha reta |
| Nadar | `Ágil` | **`1 PA`** | por metro |
| Realizar Truques | `Ágil` | **`1 PA`** | por ação |
| Esconder-se | `Ágil` | **`1 PA`** | Por ação |
| Investigar/Observar | `Mental` | **`1 PA`** | por ação |
| Ponderar | `Mental` | **`1 PA`** | por ação |
| Intimidar | `Social` | **`1 PA`** | por ação (seguindo a regra de fala em batalha) |
| Persuadir/Seduzir | `Social` | **`1 PA`** | por ação (seguindo a regra de fala em batalha) |
| Performar | `Social` | **`1 PA`** | por ação |
| Tratar/Curar | `Social` | **`2 PA`** | por ação |
| Negociar | `Social` | **`1 PA`** | por ação (seguindo a regra de fala em batalha) |

\*Outros tipos de ação não previstos neste manual podem ter seu custo definido pelo narrador com base nas ações já definidas.

## Ações complexas (em ordem alfabética)

> ⚠️ A DEFINIR: no manual, as entradas detalhadas de Agarrar, Arremessar, Correr, Empurrar, Esconder-se, Intimidar, Investigar/Observar, Levantar, Nadar, Negociar, Performar, Persuadir/Seduzir, Ponderar, Realizar Truques e Tratar/Curar existem apenas como modelos vazios (uso padrão, uso com especialização, requisitos e custo da especialização não preenchidos), e "Derrubar" não possui entrada detalhada. Apenas "Saltar" tem conteúdo, transcrito abaixo.

### Saltar

**Atributo:** `Ágil` · **Custo da técnica:** `1 exp.`

#### Uso padrão

Pode saltar:

- Quando parado: Até 2 metros
- Com impulso de corrida em linha reta: Até 2 metros + 1 metro para cada 2 metros de corrida prévia, limitado a 5 metros de salto.
- +1d10 de desvantagem para cada metro adicional após 2 metros.

#### Uso com a técnica

- Ignora os dados de desvantagem por metro adicional (a técnica apenas anula a desvantagem natural do salto; não concede outro bônus).

> ✅ Decidido em 12/07/2026: o antigo bônus de "aumentar o limite do salto para 10 metros" virou o aspecto **[Saltador](tracos-base.md#saltador)** (3 exp.; requisito: Técnica em Saltar), já que uma técnica não concede vantagem (ver [decisões — lote 2](../../notas-de-design/decisoes/2026-07-12-decisoes-de-regra-lote-2.md), item 6).

#### Requisitos de uso

- Não ter nenhuma perna [desabilitada](../conflitos/03-saude-e-protecao.md#membro-desabilitado-incapacitado-ou-invalidado).
