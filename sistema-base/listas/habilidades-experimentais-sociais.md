---
titulo: Habilidades e traços sociais (experimental)
tipo: regra
cenario: base
status: rascunho
tags: [habilidades, tracos, social, experimental]
atualizado-em: 2026-07-12
---

# `Experimental` Habilidades e traços sociais

> 🧪 Seção experimental — em teste.

Habilidades e traços voltados ao **conflito social** (malha de embate social, trilhas emocionais e campos da emoção). Como o embate social se resolve por testes de atributo **Social** ou **Mental**, esta lista reúne entradas dos dois atributos. Parte da "Lista de habilidades base" do manual base — ver o [índice de habilidades base](habilidades-base.md).

> 🧪 Lista **experimental (rascunho)**. A **escada de custo em `exp.`** e a terminologia **campo × trilha** dos efeitos abaixo só serão finalizadas quando a mecânica da **[malha de embate social](../../modulos/conflito-social/README.md)** estiver fechada (há pendência de design sobre como o movimento na malha vira pontos). As correções desta revisão foram de **consistência textual**, sem redesenhar efeitos.

## Meditação

**Atributo:** Social · **Custo de PA:** `2 PA` · **Custo de compra:** `2 exp.`

### Descrição

Se você estiver em uma posição de descontrole da malha de embate social e for bem-sucedido ao usar Recompor-se, você sobe campos adicionais em direção ao controle da trilha.

### Requisitos de uso

- Estar em uma posição de descontrole da malha de embate social.

### Progressão

1. Sobe 1 campo, tendo como ponto máximo o campo neutro da trilha emocional (aquele em que não há bônus ou penalidades);
2. Sobe 2 campos, tendo como ponto máximo o campo neutro da trilha emocional (aquele em que não há bônus ou penalidades);
3. Sobe 2 campos, e pode subir mais do que o ponto de neutralidade da trilha emocional;

## Ressignificar

**Atributo:** Mental · **Custo de PA:** `5 PA` · **Custo de compra:** `3 exp.`

### Descrição

Se o personagem estiver no campo neutro de uma trilha emocional, ele pode transitar para outra trilha diretamente, sem passar por estágios intermediários (sem passar pelos campos da emoção intermediários).

### Requisitos de uso

- Estar em posição neutra de uma trilha emocional

### Progressão

1. Pode transitar para uma trilha adjacente;
2. Pode transitar para uma trilha adjacente, ou para a seguinte;
3. Pode transitar para qualquer trilha.

## Empatia

**Atributo:** Social · **Custo de PA:** `4 PA` · **Custo de compra:** `3 exp.`

### Descrição

Ative um gatilho para se movimentar na mesma direção de um aliado quando ele se mover.

### Requisitos de uso

—

### Progressão

1. Ande um campo na direção de um aliado;
2. Ande dois campos na direção de um aliado;
3. Ande três campos na direção de um aliado.

## Otimista

**Atributo:** Social · **Custo de PA:** `5 PA` · **Custo de compra:** `3 exp.`

### Descrição

Desloca um aliado na direção do controle emocional se ele estiver na trilha da alegria.

### Requisitos de uso

- Ter um aliado na trilha da alegria.

### Progressão

1. Desloca em 1 trilha;
2. Desloca em 2 trilhas;
3. Desloca em 3 trilhas;

## Liderar pelo exemplo

**Atributo:** Social · **Custo de PA:** `4 PA` · **Custo de compra:** `3 exp.`

### Descrição

Movimente um aliado na direção do seu personagem na malha de embate social.

### Requisitos de uso

—

### Progressão

1. Desloca em 1 trilha;
2. Desloca em 2 trilhas;
3. Desloca em 3 trilhas;

## Destacar defeitos

**Atributo:** Social · **Custo de PA:** `4 PA` · **Custo de compra:** `3 exp.`

### Descrição

Desloca o adversário lateralmente na direção da ansiedade.

### Requisitos de uso

—

### Progressão

1. Desloca em 1 trilha;
2. Desloca em 2 trilhas;
3. Desloca em 3 trilhas;

## Palhaço

**Atributo:** Social · **Custo de PA:** `4 PA` · **Custo de compra:** `3 exp.`

### Descrição

Desloca o adversário lateralmente na direção da alegria

### Requisitos de uso

—

### Progressão

1. Desloca em 1 trilha;
2. Desloca em 2 trilhas;
3. Desloca em 3 trilhas;

## Sedutor

**Atributo:** Social · **Custo de PA:** `4 PA` · **Custo de compra:** `3 exp.`

### Descrição

Desloca o adversário lateralmente na direção da atração

### Requisitos de uso

—

### Progressão

1. Desloca em 1 trilha;
2. Desloca em 2 trilhas;
3. Desloca em 3 trilhas;

## Enojar

**Atributo:** Social · **Custo de PA:** `4 PA` · **Custo de compra:** `3 exp.`

### Descrição

Desloca o adversário lateralmente na direção da aversão

### Requisitos de uso

—

### Progressão

1. Desloca em 1 trilha;
2. Desloca em 2 trilhas;
3. Desloca em 3 trilhas;

## Respirar fundo

**Atributo:** Mental · **Custo de PA:** `4 PA` · **Custo de compra:** `4 exp.`

### Descrição

Impede que você seja deslocado por um oponente nos próximos turnos na malha de embate social.

### Requisitos de uso

—

### Progressão

1. Até seu próximo turno.
2. Até seu próximo turno + 1 rodada.
3. Até seu próximo turno + 2 rodadas.

## Agilizar derrota em [TRILHA]

**Atributo:** Social · **Custo de PA:** `4 PA` · **Custo de compra:** `4 exp.`

### Descrição

Adiciona **x** de pontuação negativa a um adversário no final do turno, se ele estiver em descontrole na trilha [TRILHA]

### Requisitos de uso

—

### Progressão

1. x = 1
2. x = 2
3. x = 3

## Agilizar vitória em [TRILHA]

**Atributo:** Social · **Custo de PA:** `4 PA` · **Custo de compra:** `4 exp.`

### Descrição

Adiciona x pontuação positiva a um aliado (ou você mesmo) no final do turno, se ele estiver em controle na trilha [TRILHA]

### Requisitos de uso

—

### Progressão

1. x = 1
2. x = 2
3. x = 3

## Dilema

**Atributo:** Social · **Custo de PA:** `8 PA` · **Custo de compra:** `6 exp.`

### Descrição

Movimenta todos os personagens elegíveis na malha de embate social até um dos campos da emoção adjacentes. O personagem movido dessa maneira escolhe qual campo da emoção quer ocupar.

### Requisitos de uso

—

### Progressão

1. Movimenta todos os personagens em campos de pontuação 0
2. Movimenta todos os personagens em campos de pontuação -1 e +1
3. Movimenta todos os personagens em campos de pontuação -2 e +2

## Psicólogo

**Atributo:** Social · **Custo de PA:** `1 PA` · **Custo de compra:** `3 exp.`

### Descrição

Reduz os PAs necessários para movimentar um aliado na malha de embate social.

### Requisitos de uso

—

### Progressão

1. -2 PAs do custo total da ação
2. -4 PAs do custo total da ação
3. -6 PAs do custo total da ação

## Sinergia (traço)

**Atributo:** Mental · **Custo de compra:** `3 exp.`

### Descrição

Recebe vantagem para mover um adversário na trilha de uma emoção em que você tenha um aliado.

### Requisitos de uso

- Ter ao menos 1 aliado no conflito social

## Cabeça dura (traço)

**Atributo:** Mental · **Custo de compra:** `5 exp.`

### Descrição

Tem vantagem em testes de resistência para ser tirado de uma trilha por um adversário.

### Requisitos de uso

—

## Distanciamento emocional (traço)

**Atributo:** Mental · **Custo de compra:** `3 exp.`

### Descrição

Não gasta PA para entrar em uma trilha na qual não haja nenhum outro personagem.

### Requisitos de uso

- Ter ao menos 2 oponentes no conflito social

## Sanguessuga social (traço)

**Atributo:** Social · **Custo de compra:** `4 exp.`

### Descrição

Tem vantagem em testes enquanto o oponente estiver em situação de descontrole no campo da tristeza.

### Requisitos de uso

—

## Oportunista sedutor (traço)

**Atributo:** Social · **Custo de compra:** `4 exp.`

### Descrição

Tem vantagem em testes enquanto o oponente estiver em situação de descontrole no campo da atração

### Requisitos de uso

—

## Companheiro de festa (traço)

**Atributo:** Social · **Custo de compra:** `4 exp.`

### Descrição

Tem vantagem em testes enquanto o oponente estiver em situação de descontrole no campo da alegria

### Requisitos de uso

—

## Fobófago (traço)

**Atributo:** Social · **Custo de compra:** `4 exp.`

### Descrição

Tem vantagem em testes enquanto o oponente estiver em situação de descontrole no campo da ansiedade

### Requisitos de uso

—

## Tomar vantagem da surpresa (traço)

**Atributo:** Social · **Custo de compra:** `4 exp.`

### Descrição

Tem vantagem em testes enquanto o oponente estiver em situação de descontrole no campo da surpresa

### Requisitos de uso

—

## Turrão (traço)

**Atributo:** Social · **Custo de compra:** `5 exp.`

### Descrição

Toda vez que um adversário falhar em movimentar um personagem com esse traço, este adversário desce um campo em direção ao descontrole emocional caso já esteja em alguma trilha de sentimentos.

### Requisitos de uso

—
