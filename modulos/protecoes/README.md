---
titulo: "Proteções (armaduras e escudos)"
tipo: mecanica
cenario: modulo
modulo: protecoes
status: aprovado
tags: [modulo, protecoes, armaduras, escudos, equipamentos]
atualizado-em: 2026-08-10
---

# 🧩 Módulo: Proteções (armaduras e escudos)

Catálogo das **armaduras** (por tipo/peça) e **escudos**. Módulo próprio (não faz parte da
coleção `armas`).

> **Usa o sistema do núcleo.** Como as armaduras funcionam (montagem por peça/região, redutor de
> dano e de `PA` por tipo, dano mínimo 1, uso/registro, **durabilidade**) está em
> [sistema-base — proteções](../../sistema-base/listas/protecoes-base.md) e na regra de
> [redutor de dano](../../sistema-base/conflitos/03-saude-e-protecao.md). Aqui só a **lista**.

## Armaduras (por tipo)

| **Tipo de armadura** | **Redutor Dano 🛡️** | **Redutor PA 🐢** | **Durab. (por peça)** | **Preço (por peça)** |
| --- | --- | --- | --- | --- |
| Leve | **`-1`** | **`-1 PA`** | **`10`** | **`20 moedas`** |
| Média | **`-2`** | **`-2 PA`** | **`25`** | **`60 moedas`** |
| Pesada | **`-3`** | **`-3 PA`** | **`50`** | **`140 moedas`** |

## Escudos

O **`PA` de uso** é o custo para **levantar guarda** com o escudo; o **redutor de `PA`** é a
penalidade permanente por **portá-lo**; o **redutor de dano** vale **ao defender com a guarda
levantada**.

| **Item** | **Tipo e Ações** | **Redutor Dano 🛡️** | **Redutor PA 🐢** | **Durab.** | **Preço** |
| --- | --- | --- | --- | --- | --- |
| Broquel | **`Leve`**<br>**`2 PA`** | **`-1`** | **`-1 PA`** | **`6`** | **`5 moedas`** |
| Escudo pequeno | **`Leve`**<br>**`2 PA`** | **`-1`** | **`-1 PA`** | **`10`** | **`8 moedas`** |
| Escudo médio, Cestus, Áspide | **`Média`**<br>**`3 PA`** | **`-2`** | **`-2 PA`** | **`15`** | **`14 moedas`** |
| Escudo Grande | **`Média`**<br>**`3 PA`** | **`-2`** | **`-2 PA`** | **`20`** | **`20 moedas`** |
| Escudo torre, Tater | **`Pesada`**<br>**`4 PA`** | **`-3`** | **`-3 PA`** | **`25`** | **`40 moedas`** |
